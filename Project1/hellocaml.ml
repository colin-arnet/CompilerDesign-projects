...
let centimes_of : int -> int -> int =
  fun (rp:int) -> fun (fr:int) -> rp + (mult 100 fr)
...
*)
let insert (x:'a) (l:'a list) : 'a list =
  let rec aux x left right = 
    begin match right with
    |[] -> (append (rev_t left) [x])
    |h::tl -> if(x < h) 
      then (append (rev_t left) (append [x] right)) 
      else (if (x == h) 
        then (append (rev_t left) right) 
        else (aux x (h::left) tl))
    end
  in
  aux x [] l
...
let rec union (l1:'a list) (l2:'a list) : 'a list =
  begin match l1, l2 with
    ...
    |h1::tl1, h2::tl2 -> if(h1 < h2) 
      then (append [h1] (union tl1 l2)) 
      else if(h1 == h2) 
        then (append [h1] (union tl1 tl2)) 
        else (append [h2] (union l1 tl2))
  end
...
let rec lookup (x:string) (c:ctxt) : int64 =
  begin match c with
    |[] -> raise Not_found
    |(s, n)::tl -> if(x = s) then n else (lookup x tl)
  end
...
let rec optimize (e:exp) : exp =
  begin match e with
    |Var _ -> e
    |Const _ -> e
    |Add (Const 0L, Const 0L) -> Const 0L
    |Add (Const 0L, Const x) -> Const x
    |Add (Const x, Const 0L) -> Const x
    |Add (Const x, Const y) -> Const (Int64.add x y)
    |Add (Const 0L,g) -> (optimize g)
    |Add (g, Const 0L) -> (optimize g)
    |Add (g, f) -> if ((optimize g) = g && (optimize f) = f) then e else optimize (Add(optimize g, optimize f))
    |Mult (Const 0L, Const 0L) -> Const 0L
    |Mult (Const 0L, Const _) -> Const 0L
    |Mult (Const _, Const 0L) -> Const 0L
    |Mult (Const 1L, f) -> f
    |Mult (f, Const 1L) -> f
    |Mult (Const x, Const y) -> Const (Int64.mul x y)
    |Mult (Const 0L, e) -> Const 0L
    |Mult (g, Const 0L) -> Const 0L
    |Mult (g, f) -> if ((optimize g) = g && (optimize f) = f) then e else optimize (Mult(optimize g, optimize f))
    |Neg (Const 0L) -> Const 0L
    |Neg (Const x) -> Const (Int64.neg x)
    |Neg f -> if ((optimize f) = f) then e else optimize (Neg (optimize e))
  end
...
let rec compile (e:exp) : program =
  begin match e with
    |Var s -> [IPushV s]
    |Const x -> [IPushC x]
    |Add (f ,g) -> (append (compile f) (append (compile g) [IAdd]))
    |Mult (f, g) -> (append (compile f) (append (compile g) [IMul]))
    |Neg f -> (append (compile f) [INeg])
  end
