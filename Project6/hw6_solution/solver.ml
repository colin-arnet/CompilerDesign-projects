...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) =
  struct
    ...
    let rec loop (g : Graph.t) (w : Graph.NodeS.t) : Graph.t =
      if(Graph.NodeS.is_empty w) then
        g
      else
        (* w.pop() *)
        ...
        let pred_list = Graph.NodeS.elements preds in
        let pred_fact_list = List.map (Graph.out g) pred_list in
        let pred_fact = Fact.combine pred_fact_list in
        (* out[n] = flow[n] (preds) *)
        let flow_fact = Graph.flow g n pred_fact in
        let new_g = Graph.add_fact n flow_fact g in
        (* if(!equal old_out out[n]) *)
        let w = if ((Fact.compare old_out (Graph.out new_g n)) <> 0) then
          (* for all m in succs[n]; *)
          let succs_set = Graph.succs g n in
          (* w.add(m) *)
          Graph.NodeS.union w succs_set
        else
          w
        in
        loop new_g w
