...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
   ...
   *)
  | Ast.CStruct (id, l) ->
    cmp_ast_cstruct (id,l) tc c exp

  | Ast.Proj (e, id) ->
    cmp_ast_proj (e,id) tc c exp
    

and cmp_ast_proj (e,id) (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream = 
  let ans_ty, ptr_op, code = cmp_exp_lhs tc c exp in
  let ans_id = gensym "proj" in
  ans_ty, Id ans_id, code >:: I(ans_id, Load(Ptr ans_ty, ptr_op))

and cmp_ast_cstruct (id,l) (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream = 
...
and cmp_ast_length e (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream = 
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    cmp_ast_cast (typ, id, exp, notnull, null) (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node)
...
and cmp_ast_cast (typ, id, exp, notnull, null) (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream = 
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    cmp_cstruct (id,cs) c (tc : TypeCtxt.t) (e:Ast.exp node)

  | _ -> failwith "bad global initializer"

and cmp_cstruct (id,cs) c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list = 
